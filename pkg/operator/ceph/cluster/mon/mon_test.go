...
func TestSaveMonEndpoints(t *testing.T) {
	...
	assert.Nil(t, err)
	assert.Equal(t, "rook-ceph-mon-a=1.2.3.1:6790", cm.Data[EndpointDataKey])
	assert.Equal(t, `{"node":{},"port":{}}`, cm.Data[MappingKey])
	assert.Equal(t, "-1", cm.Data[MaxMonIDKey])

	// update the config map
	c.clusterInfo.Monitors["rook-ceph-mon-a"].Endpoint = "2.3.4.5:6790"
	c.maxMonID = 2
	...
	cm, err = c.context.Clientset.CoreV1().ConfigMaps(c.Namespace).Get(EndpointConfigMapName, metav1.GetOptions{})
	assert.Nil(t, err)
	assert.Equal(t, "rook-ceph-mon-a=2.3.4.5:6790", cm.Data[EndpointDataKey])
	assert.Equal(t, `{"node":{"rook-ceph-mon-a":{"Name":"node0","Hostname":"myhost","Address":"1.1.1.1"}},"port":{"node0":12345}}`, cm.Data[MappingKey])
