...
func TestCheckHealthNotFound(t *testing.T) {
	...
	assert.Nil(t, err)
	if cm.Data[EndpointDataKey] != "rook-ceph-mon-a=1.2.3.1:6790,rook-ceph-mon-b=1.2.3.2:6790" {
		assert.Equal(t, "rook-ceph-mon-b=1.2.3.2:6790,rook-ceph-mon-a=1.2.3.1:6790", cm.Data[EndpointDataKey])
	}
	...
	cm, err = c.context.Clientset.CoreV1().ConfigMaps(c.Namespace).Get(EndpointConfigMapName, metav1.GetOptions{})
	assert.Nil(t, err)
	if cm.Data[EndpointDataKey] != "rook-ceph-mon-a=1.2.3.1:6790,rook-ceph-mon-f=:6790" {
		assert.Equal(t, "rook-ceph-mon-f=:6790,rook-ceph-mon-a=1.2.3.1:6790", cm.Data[EndpointDataKey])
	}
}
